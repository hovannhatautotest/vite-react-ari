...
CRU-05 Verify that validation text of "Họ và tên" field display when create new user with "Họ và tên" field empty
    [Tags]          @smoketest         @regression
    Go to page create user
...
CRU-06 Verify that validation text of "Email" field display when create new user with "Email" field empty
    [Tags]          @smoketest         @regression
    Go to page create user
...
CRU-23 Verify that Create user unsuccessfully because email is already taken & "lưu lại" button
    ...
    And Enter "email" in "Email" with "hovannhat_staff@gmail.com"
...
#-------------------------------------VIEW LIST OF USER--------------------------------------------------------------------------------------
CRU-35 Verify that the list of users all Role can be viewed successfully
    Login to admin
    And Click "Người Dùng" menu
    Then Show list of users

CRU-36 Verify that user can view the list of Staff successfully
    Login to admin
    And Click "Người Dùng" menu
